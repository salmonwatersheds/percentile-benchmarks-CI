...
# modelBoot
tau <- 0.6
simDat <- simSpawners(seed = 3958, tau = tau)
nBoot <- 10000
spawners <- simDat[2:(n + 1), 'spawners']

S25 <- quantile(spawners, 0.25)
S50 <- quantile(spawners, 0.50)

# True CI?
trueParamSpawners <- matrix(nrow = n, ncol = nBoot)
trueBench <- matrix(nrow = nBoot, ncol = 2)
for(i in 1:nBoot){
	simDat.i <- simSpawners(tau = tau)
	trueParamSpawners[, i] <- simDat.i[2:(n + 1), 'spawners']
	trueBench[i, ] <- quantile(trueParamSpawners[, i], c(0.25, 0.5))
}
trueCI <- apply(trueBench, 2, quantile, c(0.025, 0.975))


CI.all <- list(
	naiveCI = blockBoot(series = spawners, blockLength = 1, nBoot = nBoot),
	block5CI = blockBoot(series = spawners, blockLength = 5, nBoot = nBoot),
	block10CI = blockBoot(series = spawners, blockLength = 10, nBoot = nBoot),
	block15CI = blockBoot(series = spawners, blockLength = 15, nBoot = nBoot),
	modelCI = modelBoot(series = spawners, numLags = 1, nBoot = nBoot)
)
...
par(mar = c(1,1,0,0), oma=c(2,2,4,1))
for(i in 1:3){
	for(j in 1:5){
		plot(1:n, spawners*10^-3, "n", xaxt="n", las=1, ylab="", xlab="", yaxt="n")
		lines(1:n, spawners*10^-3, col=grey(0.8), lwd=2)#, "o", pch=21, bg="white")
		abline(h = c(S25, S50)* 10^-3, col= paste(statusCols[c('r', 'g')], 30, sep=""), lwd=2)
