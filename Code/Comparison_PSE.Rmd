...
In practice, this is not a problem for the CUs whose biostatus is assessed with the "sr" benchmarks, or is "data-deficient" or "not-assessed". Here below are the CUs with their biostatus determined by the percentile benchmarks:
...
It seems that the issue is observed either because of (i) a steady increase or decrease of the time series or (2) missing data that could generate relatively high increments between consecutive sub-series.
...
NOT FIXED. Could be due to individual data points which are on the lower side. I do not know how individual data points are handled in the Yule-Walker method used to fit the auto-regression model.
